...
class GridListExampleSimple extends React.Component{
    ...
    fetchProducts=(skip=0) =>{
        ...
        request.get('/products?index='+skip).end((err, res) => {
            if (err || !res.body) {
                console.error("Oh no!" + err);
                return;
            }
                ...
                this.setState({
                    ...
                    tilesData:this.state.tilesData.concat(res.body.result),
    ...
    loadMoreItems = (page) => {
        this.fetchProducts(this.page * this.state.limit);
        this.page++;
    }
    ...
    render() {
        ...
        const rows=this.state.tilesData.map((tile) => (
            <GridTile
                key={tile.image_urls}
                title={tile.name}
                subtitle={<span>price <b>{tile.price}</b></span>}
                actionIcon={<IconButton onClick={() => {
                    this.props.history.push('/app/' + tile.id)
        ...
        return (
            ...
            <div style={styles.root}>
                <Subheader>Showing {(this.page-1)*this.state.limit} of {this.state.result_length} products</Subheader>
...
export default withRouter(GridListExampleSimple);